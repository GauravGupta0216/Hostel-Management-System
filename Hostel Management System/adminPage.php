<?php
	...
	if(isset($_POST['submit1'])){
		...
		$query = "SELECT flag_occupied FROM ROOM WHERE room_no='$roomNo' AND hostel_name='$hostelName' AND admin_id='$admin_id'" ;
		...
		if($rows == 1) {
			...
			if($flag == 0){
				//delete vacant room
				$run = mysqli_query($con,"DELETE FROM ROOM WHERE room_no='$roomNo' AND hostel_name='$hostelName' AND admin_id='$admin_id'") ;
			}
			if($flag == 1){
				//occupied room
	...
	if(isset($_POST['submit2'])){
		...
		if($rows == 1) {
			$query = "INSERT INTO ROOM VALUES ('$roomNo','$hostelName',0,'$admin_id',NULL)" ;
			$run = mysqli_query($con,$query) ;
			if($run)
				?> <script> alert('Room Added!') ; </script> <?php
		}
		else{
			?> <script> alert('You are not allowed to add rooms to this hostel!') ; </script> <?php
		}
	}
		...
		$run = mysqli_query($con,"SELECT * FROM APPLICATION_FORM WHERE applicant_roll='$roll'") ;
			...
			if($rows == 1){
				...
				if($rows == 1) {
					...
					$run = mysqli_query($con,"SELECT * FROM ROOM WHERE hostel_name='$hostelCh' AND flag_occupied=0") ;
					$rows = mysqli_num_rows($run) ;
